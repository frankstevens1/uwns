...
import { getActionPlatformConfig, getPlatformBadgeStyle } from "./utils";
...
export function ActionPlatforms({
  ...
  React.useEffect(() => {
    ...
    if (openPlatformKey === totalsAccordionValue) {
      const platformCount = new Set(
        actions.map((action) => action.platform),
      ).size;
      if (platformCount < 5) {
    ...
    updateOpenPlatformKey(null);
  }, [openPlatformKey, actions, updateOpenPlatformKey]);
  ...
  return (
    <Card padding="none" elevation="sm" variant="subtle">
      <CardHeader>
        <div className="text-sm font-medium">Action platforms</div>
      </CardHeader>
      <CardBody padding="none">
        {actions.length === 0 ? (
          <p className="px-4 py-5 text-sm text-(--ui-muted-fg)">
            No action platforms yet.
          </p>
        ) : (
          <AccordionRoot
